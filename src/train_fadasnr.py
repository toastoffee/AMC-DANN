...
from dataset import fadasnr_dataset
...
def train(model:        fada.FADA,
          fada_dataset: fadasnr_dataset.FadaSnrDataset,
          batch_size:   int,
          seed:         int,
          device:       torch.device,
          model_name:   str) -> None:
...
if __name__ == "__main__":
    ...
    for shots in shots_selections:
        for i in range(3):
            dataset = fadasnr_dataset.FadaSnrDataset(s_ds, t_ds, 0.6, shots, 1)
            device: torch.device = get_device()
            model = fada.FADA()
            train(model, dataset, 512, 42, device, f"fada_weights/fadasnr_shots-{shots}_round-{i}")
