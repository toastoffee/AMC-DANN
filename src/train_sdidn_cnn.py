...
from module.sdidn_cnn import DistanDANN
...
def run_train(da_dataset: str, model_name: str, seq: int):
    ...
    model.g.load_state_dict(torch.load(f"../autodl-tmp/uda/{da_dataset}/sdidn_cnn_pretrain/" + f'pretrained_sdidn_encoder.pth'))
    model.reconstructor.load_state_dict(torch.load(f"../autodl-tmp/uda/{da_dataset}/sdidn_cnn_pretrain/" + f'pretrained_sdidn_decoder.pth'))
    ...
    orthogonal_criterion = CovarianceOrthogonalLoss
    classification_criterion = nn.CrossEntropyLoss().to(device)
    contrastive_criterion = DomainContrastiveLoss
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, ((source_data, source_labels, source_snr, source_domains),
            ...
            (class_logits, domain_logits, features_class,
             features_domain, features_domain_sum, recons, domain_logits_from_upper) = model(combined_data, alpha)
...
def validate_model(model, valid_loader, device):
    ...
    with torch.no_grad():
        for data, labels, snr, domains in valid_loader:
            ...
            class_logits, _, _, _, _, _, _ = model(data, 1.0)
